...
function f8() {
    ...
    let raceName_one = document.querySelector('.s-81').value;
    let raceName_two = document.querySelector('.s-82').value;
    const requestHeaders = new Headers();
    requestHeaders.append("apikey", APIKEY);


    let promise_one = new Promise((resolve) => {
        fetch(URL + '/api/27/sr/read/' + raceName_one, {
            method: 'GET',
            headers: requestHeaders,
        })
            .then(response => resolve(response.json()))
    ...
    let promise_two = new Promise((resolve) => {
        fetch(URL + '/api/27/sr/read?race=' + raceName_two, {
            method: 'GET',
            headers: requestHeaders,
        })
            .then(response => resolve(response.json()))
    ...
    Promise.all([promise_one, promise_two])
        .then(data => {
            console.log(data);
            for (const key in data) {
                let imageRace = document.createElement('img');
                let srcString = `${URL}${data[key].result.image}`
                imageRace.setAttribute('src', srcString);
                innerDiv.append(imageRace);
            }
