...
function t2() {
    return a2_res = a2.map(item => Math.pow(item, 2))
}
...
function t3() {
    return a3_res = a3.map(item => +item)
}
...
function t8() {
    return a8_res = a8.map((item, index, arr) => {
        return item % 2 === 0 ? arr.indexOf(item) : false;
    }).filter(item => item);
}
...
function t9() {
    let arr = []
    a9.forEach(item => arr[arr.length] = item.toLowerCase())
    return a9 = arr;
}
...
function t10() {
    let arr = [];
    a10.forEach((item, index, array) => {
        return arr[arr.length] = array[index] > 0 ? array[index] : 0;
    })
    return a10 = arr;
}
...
function t11() {
    return a11_res = a11.filter((item, index) => index % 2 === 0);
}
...
function t14() {
    let lowerCase = a14.includes(a14_sym.toLowerCase());
    let upperCase = a14.includes(a14_sym.toUpperCase());
...
function t15() {
    return a15_res = a15.filter((item) => {
