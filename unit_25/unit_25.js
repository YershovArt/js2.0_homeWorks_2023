...
function f1() {
	...
	xhr.onload = function () {
		...
		document.querySelector('.out-1').textContent = data['random-string'];
	}
	xhr.send();
}
...
function f4() {
	...
	xhr.onload = function () {
		const data = JSON.parse(xhr.response);
		document.querySelector('.out-4').textContent = data['random-number'];
	}
...
function f5() {
	...
	const dataOnServer = new FormData();
	dataOnServer.append('min', min);
	dataOnServer.append('max', max);

	xhr.onload = function () {
		const data = JSON.parse(xhr.response);
		document.querySelector('.out-5').textContent = data['random-number'];
	}

	xhr.send(dataOnServer)
}
...
function f6() {
	...
	xhr.onload = function () {
		...
		document.querySelector('.out-6-status').textContent = status;
		document.querySelector('.out-6-response').textContent = text;
	}
...
function f7() {
	...
	xhr.onload = function () {
		const data = JSON.parse(xhr.response);
		let srcImage = `${URL}${data.result.image}`;
...
function f8() {
	...
	xhr.onload = function () {
		const data = JSON.parse(xhr.response);
		document.querySelector('.out-8').textContent = data.itemCount;
	}
...
function f9() {
	let id = document.querySelector('.i-9').value;

	const xhr = new XMLHttpRequest();
	xhr.open('POST', URL + '/api/25/employee/read/' + id);

	xhr.setRequestHeader('apikey', APIKEY);

	xhr.onload = function () {
		const data = JSON.parse(xhr.response);
		document.querySelector('.out-9').textContent = data.result.designation;
	}
...
function f10() {
	let id = document.querySelector('.i-10').value;

	const xhr = new XMLHttpRequest();
	xhr.open('GET', URL + '/api/25/employee/read/' + id);
...
function f11() {
	let length = document.querySelector('.i-11').value;

	const xhr = new XMLHttpRequest();
	xhr.open('POST', URL + '/api/25/random/generate-password');
	xhr.setRequestHeader('apikey', APIKEY);

	let dataOnServer = new FormData();
	dataOnServer.append('length', length)
...
function f12() {
	...
	let checkBoxState = document.querySelector('.ch-12').checked;
	...
	let dataOnServer = new FormData();
	dataOnServer.append('length', length)
	dataOnServer.append('symbols', checkBoxState || 0);
...
function f13() {
	...
	let checkBoxState = document.querySelector('.ch-131').checked;
	let secondCheckBoxState = document.querySelector('.ch-132').checked;
	...
	dataOnServer.append('length', length)
	dataOnServer.append('symbols', checkBoxState || 0);
	dataOnServer.append('uppercase', secondCheckBoxState || 0);
...
function f15() {
	...
	xhr.onload = function () {
		...
		for (const item of data.result) {
			let srcString = `${URL}${item.image}`;
			let img = document.createElement('img')
			img.setAttribute('src', srcString);
			divForIMG.append(img)
		}

		document.querySelector('.out-15').append(divForIMG);
	}
...
function f17() {
	let worldFind = document.querySelector('.s-17').value;
	const xhr = new XMLHttpRequest();

	xhr.open('GET', URL + '/api/25/gow/world/' + `${worldFind}`);
	xhr.setRequestHeader('apikey', APIKEY);
...
function f18() {
	...
	xhr.onload = function () {
		...
		for (const key in data.rune) {
			let srcImage = `${URL}${data.rune[key]}`;
		...
		document.querySelector('.out-18').append(divForImage);
	}
