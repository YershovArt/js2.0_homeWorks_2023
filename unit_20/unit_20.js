...
function t3(event) {
    document.querySelector('.out-3').textContent += `${event.keyCode >= 48 && event.keyCode <= 57 ? false : true} `;
}
...
function t4(event) {
    ...
    document.querySelector('.out-4').textContent += `${event.keyCode >= 65 && event.keyCode <= 90 ? '' : event.key}`;
}
...
function t6(event) {
    ...
    str += event.keyCode >= 65 && event.keyCode <= 90 ? '' : event.key;
    i6.value = str;
    return false;
}
...
function t7() {
    let i = Math.floor(Math.random() * ((a7.length - 1) - 0 + 1) + 0);
...
function t8(event) {
    ...
    document.querySelector('.out-8').textContent += a8[event.key] === 0 ? 0 : a8[event.key] ? a8[event.key] : event.key;
}
...
function t9(event) {
    console.log(event);
    event.key === `ArrowDown` ? arr.push(1) : false;
    document.querySelector('.out-9').textContent = arr.length;
}
...
function t11(event) {
    ...
    for (const item of keyBoard) {
        ...
        } else if (special[item.getAttribute('data')] === event.key){
            item.classList.add('active');
        } else {
